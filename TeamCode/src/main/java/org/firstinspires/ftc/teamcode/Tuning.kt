...
class Tuning :
    SelectableOpMode("Select a Tuning OpMode", Consumer { s: SelectScope<Supplier<OpMode?>?>? ->
        ...
        s.folder("Swerve", Consumer { p: SelectScope<Supplier<OpMode?>?>? ->
...
internal class ForwardTuner : OpMode() {
    ...
    override fun loop() {
        ...
        telemetryM.debug(
            "Multiplier: " + (DISTANCE / (follower.getPose().getX() / follower.getPoseTracker()
...
internal class LateralTuner : OpMode() {
    ...
    override fun loop() {
        ...
        telemetryM.debug(
            "Multiplier: " + (DISTANCE / (follower.getPose().getY() / follower.getPoseTracker()
    ...
    override fun loop() {
        ...
        if (!end) {
            if (Math.abs(follower.getPose().getY()) > (DISTANCE + 72)) {
                ...
                val currentVelocity: Double =
                    Math.abs(follower.getVelocity().dot(Vector(1.0, Math.PI / 2)))
                velocities.add(currentVelocity)
                velocities.removeAt(0)
            }
        } else {
...
internal class AnalogMinMaxTuner : OpMode() {
    ...
    var encoderNames: Array<String?> = arrayOf<String?>(
        "leftFrontEncoder",
        "rightFrontEncoder",
        "leftBackEncoder",
        "rightBackEncoder"
    )
    var encoders: Array<AnalogInput?> = arrayOfNulls<AnalogInput>(encoderNames.size)
    ...
    override fun init() {
        lynxModules = hardwareMap.getAll<LynxModule?>(LynxModule::class.java)
        for (hub in lynxModules!!) {
            hub.setBulkCachingMode(LynxModule.BulkCachingMode.MANUAL)
        }

        for (i in encoders.indices) {
            encoders[i] = hardwareMap.get<AnalogInput?>(AnalogInput::class.java, encoderNames[i])
    ...
    override fun loop() {
        ...
        for (i in encoders.indices) {
            val currentVoltage = encoders[i]!!.getVoltage()
...
internal object Drawing {
    ...
    fun drawRobot(pose: Pose?, style: Style = robotLook) {
        if (pose == null || Double.isNaN(pose.getX()) || Double.isNaN(pose.getY()) || Double.isNaN(
                pose.getHeading()
            )
        ) {
            return
